...
def main():
    ...
    parser.add_argument('-r', '--renewall', action='store_true',
        help='crawl from continue existed data to now')
    ...
    if args.back or args.check:
        ...
        s0050=pd.read_csv('data/0050.csv', header=None)
        s0050.columns=['s0','s1','s2','s3','s4','s5','s6','s7','s8']
        pattern = re.compile(r'\d+') 
        old_last_day = pattern.findall(s0050['s0'].values[-1])
        old_last_day = datetime(int(old_last_day[0])+1911,int(old_last_day[1]),int(old_last_day[2]))
        max_error = 5
        error_times = 0        
        while error_times < max_error and first_day > old_last_day:
            old_last_day+=timedelta(1)
            try:
                crawler.get_data((old_last_day.year, old_last_day.month, old_last_day.day))
                error_times = 0
            except:
                print('warning:'+str('continue day'+1+'cant get data'))
                date_str = old_last_day.strftime('%Y/%m/%d')
                logging.error('Crawl raise error {}'.format(date_str))
                error_times += 1
                continue
    else:
